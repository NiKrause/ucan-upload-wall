 ...
 */
export function decodeWebAuthnVarsigV1(varsig: Uint8Array): DecodedVarsigV1 {
  if (varsig.length < 2) {
  ...
  const algorithm: SignatureAlgorithm | null =
    innerAlgorithm === INNER_EDDSA ? 'Ed25519'
      : innerAlgorithm === INNER_ECDSA ? 'P-256'
      : null;

  if (!algorithm) {
    throw new Error(`Unsupported signature algorithm: 0x${innerAlgorithm.toString(16)}`);
  }
  ...
  if (multihashCode !== MULTIHASH_SHA256 || multihashLength !== MULTIHASH_SHA256_LEN) {
